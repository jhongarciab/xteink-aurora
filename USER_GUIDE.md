...
Welcome to the **xAurora** firmware. This guide outlines the hardware controls, navigation, and reading features of the device.

- [xAurora User Guide](#xaurora-user-guide)
...
xAurora supports sending books from Calibre using the CrossPoint Reader device plugin.
...
- **UI Theme**: Set which UI theme to use:
  ...
  - "Lyra xAurora" - xAurora's 3-book Home carousel with reading progress
...
xAurora can sync reading progress with KOReader-compatible sync servers.
...
2. On each xAurora device:
   - Go to **Settings -> System -> KOReader Sync**.
   - Set **Username** and **Password** (enter the plain password; xAurora computes MD5 internally, and use the same values on all devices).
...
xAurora authenticates against KOReader Sync (`koreader/kosync`) using an MD5 key, so register using the MD5 of your password:
...
| Mode | Behavior |
|------|----------|
| **Dark** (default) | The xAurora logo on a dark background. |
| **Light** | The xAurora logo on a white background. |
...
xAurora renders text using the following Unicode character blocks, enabling support for a wide range of languages:
