...
| Current release (CPR-vCodex) build | [`1.3.0.20-cpr-vcodex`](https://github.com/franssjz/cpr-vcodex/releases/tag/1.3.0.20-cpr-vcodex) |
...
| Current release fixes | Improves dictionary navigation responsiveness, keeps OPDS per-server download directories reusable, adds Turkish hyphenation/casefolding support, and stops Reading Heatmap month changes from forcing clean screen refreshes. |
| Latest release notes | - Cached reader dictionary word-selection redraws so moving between candidate words stays responsive on X3/X4-class hardware.<br>- Reused active dictionary scan results in the Dictionary app to avoid expensive SD-card rescans with many installed dictionaries.<br>- Fixed OPDS per-server download directory reuse from [#97](https://github.com/franssjz/cpr-vcodex/pull/97).<br>- Integrated Turkish hyphenation/casefolding and Istanbul time zone support from [#88](https://github.com/franssjz/cpr-vcodex/pull/88).<br>- Switched Reading Heatmap left/right month navigation to fast refresh. |
...
- base firmware line: `CrossPoint Reader 1.3.0`
- current release build style: `1.3.0.20-cpr-vcodex`
...
```powershell
python scripts/pre_release_check.py --tag 1.3.0.20-cpr-vcodex
```

- push a stable tag named like `1.3.0.20-cpr-vcodex`
