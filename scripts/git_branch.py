"""
PlatformIO pre-build script for xAurora versioning.
...
def _extract_release_number(base_version, text):
    match = re.search(rf"{re.escape(base_version)}\.(\d+)-xaurora", text)
...
def _latest_tag_release_number(project_dir, base_version):
    try:
        tags = subprocess.check_output(
            ["git", "tag", "--list", f"{base_version}.*-xaurora"],
            text=True,
...
def release_number_from_tag(base_version):
    tag = os.environ.get("XAURORA_RELEASE_TAG") or os.environ.get("GITHUB_REF_NAME")
    if not tag:
        return None

    match = re.fullmatch(rf"{re.escape(base_version)}\.(\d+)-xaurora", tag)
    if not match:
        raise ValueError(f"Release tag {tag!r} does not match expected pattern {base_version}.<release>-xaurora")
...
def write_version_artifacts(
    ...
    *,
    ...
    inc_content = (
        ...
        f"const char XAURORA_BASE_VERSION[] = {_cpp_string_literal(base_version)};\n"
        f"const int XAURORA_BUILD_SEQ = {build_counter};\n"
        f"const int XAURORA_RELEASE_SEQ = {release_number};\n"
        f"const char XAURORA_BUILD_KIND[] = {_cpp_string_literal(build_kind)};\n"
    )
...
def inject_version(env):
    ...
    if env_name == "default":
        ...
        build_kind = "dev"
        print(f"xAurora release line: {release_number} ({release_counter_path})")
    ...
    print(f"xAurora build version: {version_string}")
    print(f"xAurora {build_kind} counter: {build_counter} ({counter_path})")
