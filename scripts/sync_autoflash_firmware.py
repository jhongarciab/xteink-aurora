...
DEFAULT_REPO = "jhongarciab/xteink-aurora"
APP_PARTITION_SIZE = 6_553_600
MIN_FIRMWARE_SIZE = 1_000_000
VERSION_RE = re.compile(r"\b\d+\.\d+\.\d+\.\d+(?:[.-][0-9A-Za-z]+)?-[0-9A-Za-z._-]*xaurora\b")
FIRMWARE_TAG_RE = re.compile(r"^\d+\.\d+\.\d+\.\d+(?:[.-][0-9A-Za-z]+)?-xaurora$")
...
def request_json(url: str, token: str | None) -> Any:
    headers = {
        ...
        "User-Agent": "xaurora-autoflash-sync",
    }
    if token:
...
def download_bytes(url: str) -> bytes:
    request = urllib.request.Request(url, headers={"User-Agent": "xaurora-autoflash-sync"})
...
def firmware_tag_sort_key(tag: str) -> tuple[int, int, int, int, int, str] | None:
    ...
    prefix = tag.removesuffix("-xaurora")
...
def fetch_firmware_release_by_tag(repo: str, tag: str, token: str | None) -> dict[str, Any]:
    ...
    if not FIRMWARE_TAG_RE.fullmatch(str(release.get("tag_name", ""))):
        raise RuntimeError(f"Release {tag} is not a xAurora firmware release")
...
def fetch_latest_firmware_release(repo: str, token: str | None) -> dict[str, Any]:
    ...
    raise RuntimeError("Could not find a stable xAurora firmware release")
